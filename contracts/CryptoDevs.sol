...
contract CryptoDevs is ERC721Enumerable,Ownable(msg.sender){
    ...
    constructor (address whitelistContract) ERC721("CryptoDevs", "CDV") {
        whitelist = Whitelist(whitelistContract);
        reservedTokens = whitelist.maxWhitelistedAddresses();
    }

    function mint() public payable{
        require(totalSupply() + reservedTokens - reservedTokensClaimed < maxTokenIds, "EXCEEDS_MAX_SUPPLY");

        if(whitelist.whitelistedAddresses(msg.sender) && msg.value < _price){
            require( balanceOf(msg.sender) == 0, "ALREADY_OWNER");
